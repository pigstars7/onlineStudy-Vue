  ...
  <header id="topbar">
    <el-row>
      <el-col :span="4" class="topbar-left">
        <i class="iconfont el-icon-s-tools"></i>
        <span class="title" @click="index()">在线教学平台管理系统</span>
      </el-col>
      <el-col :span="20" class="topbar-right">
        <i class="el-icon-menu" @click="toggle()"></i>
        <div class="user">
          <span>{{user.userName}}</span>
          <img src="https://i.loli.net/2021/03/24/HGiRfuEZPn2XmWq.jpg" class="user-img" ref="img" @click="showSetting()" />
          <transition name="fade">
            <div class="out" ref="out" v-show="login_flag">
              <ul>
              <!-- <li><a href="javascript:;">用户信息</a></li> -->
                <li  @click="changePwd()"><a href="javascript:;">修改密码</a></li> 
...
export default {
  ...
  methods: {
    ...
    getUserInfo() { //获取用户信息
      ...
      let userId = this.$cookies.get("did")
      this.user.userName = userName
      this.user.userId = userId
    },
    changePwd() {
    this.$router.push({path: '/tpwdChaneg'})
    },
    exit() {
      let role = this.$cookies.get("role")
      this.$router.push({path:"/"}) //跳转到登录页面
      this.$cookies.remove("cname") //清除cookie
      this.$cookies.remove("did")
